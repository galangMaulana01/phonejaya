...
process.on('uncaughtException', (err) => {
  ...
  // CRITICAL: attaching this listener disables Node's default behavior of
  // exiting immediately on an uncaught exception. Without this explicit
  // exit, the process just hangs (with no response ever sent) until the
  // full function timeout kills it - turning a fast failure into a
  // multi-minute hang. Always exit after cleanup here.
  process.exit(1);
});
