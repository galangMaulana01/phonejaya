...
middleware, read directly from source), **not** by hitting the live deployment.
...
issue. Every finding below is a **logic/code finding**, marked `NEEDS LIVE VERIFICATION` where a live
request would be the natural next step to confirm real-world impact once network access is available.

Per instructions for this pass: **no fixes were applied.** This file only records findings.
...
- **Fix direction (not applied):** restrict `jabatan` choices available to `kepala_cabang` callers to non-owner roles at the route/service layer, independent of the schema-level literal validation.
...
- **NEEDS LIVE VERIFICATION** once network access is restored: fire a test employee, confirm their pre-fire token still works.
...
- **Contrast:** `transfer_stok_service.respond_transfer` and `request_sparepart_service.approve_request` correctly use `find_one_and_update` as an atomic claim gate; this handler should use the same pattern before calling `kurangi_stok_batch`.
...
- **Note:** the list endpoint `GET /transaksi` correctly scopes to the caller's cabang — only the single-record detail endpoint is missing the check.
...
- **Contrast:** `list_service` (the collection endpoint) correctly forces `cabang = user.cabang` for non-owners — the single-record endpoints simply forgot the equivalent check.
...
- **NEEDS LIVE VERIFICATION**: this should reproduce immediately and deterministically once network access is available — call `GET /cod/kurir-list` as kasir/bayu123 and confirm an empty list despite active kurir accounts existing.
...
- **Contrast:** the Instagram path extracts a shortcode and always builds its own `instagram.com` URL — TikTok has no equivalent safeguard.
...
|----|----------|------|----------|
| BUG-001 | Critical | Karyawan | kepala_cabang can create an Owner account |
| BUG-002 | High | Auth | Fired employee's token still works up to 7 days |
| BUG-003 | High | Service | Concurrent PUT can double-decrement sparepart stock |
| BUG-004 | High | Transaksi | Detail endpoint leaks cross-branch financial data |
| BUG-005 | High | Service | Detail endpoints leak cross-branch data to kurir/influencer |
| BUG-006 | High | COD | "Kurir" vs "kurir" casing breaks jual COD + dropdown |
| BUG-007 | High | Influencer | SSRF via unvalidated TikTok video URL |
| BUG-008 | Medium | Karyawan | Non-atomic username check-then-insert race |
| BUG-009 | Medium | Transfer Stok | Partial-failure leaves units migrated with no rollback |
| BUG-010 | Medium | Transaksi | Unit stranded "Sold" on partial transaction failure |
| BUG-011 | Medium | Transaksi | Partial sparepart decrement not reversed on later failure |
| BUG-012 | Medium | COD | Overlapping reject endpoints bypass mandatory reason |
| BUG-013 | Medium | COD | Monitoring aggregation drops delivery-type stats |
| BUG-014 | Medium | COD | Generic status endpoint bypasses submit-beli, loses deal_price |
| BUG-015 | Medium | Influencer | Synchronous unbounded TikTok fetch can hang request |
| BUG-016 | Medium | Upload | DELETE /upload/image has no ownership check |
| BUG-017 | Low | Units | Modal-history endpoint is dead code, always empty |
| BUG-018 | Low | Transaksi | Duplicate trx_id generation wastes sequence numbers |
| BUG-019 | Low | Customer | Stale rejection metadata survives resubmit→approve |
| BUG-020 | Low | Customer | Concurrent approve/reject duplicates status_history |
| BUG-021 | Low | Influencer | Cron secret compared non-constant-time |
| BUG-022 | Low | Transfer Stok | notif/count leaks pending-transfer count to influencer/kurir |
